...
2. Refresh each file session with `termous.sftp.sessions.get` immediately before requesting approval and retain its current `connection_generation`.
3. For downloads and remote copies, inspect every remote source with `stat`. Upload sources are Core-local paths and are validated by Termous during grant creation and again after approval; do not try to inspect them with an SFTP tool.
...
4. Call the upload tool and wait for native approval. Termous validates and grants the local paths; do not try to create or reuse a local grant directly.
...
4. Call the download tool and wait for native approval. Do not treat an MCP-side confirmation as a substitute.
...
4. Call the tool once and wait for native approval. Do not emulate this operation with a local download followed by an upload.
...
4. Never start another transfer automatically after cancellation or failure. MCP-created transfers are intentionally not retryable; a new attempt requires a new user decision and native approval.
